...
// Config is interface config of vlan slave.
type Config struct {
	MasterName   string
	IfName       string
	Vid          int
...
func (c *Config) Setup(netNS ns.NetNS) error {
	master, err := netlink.LinkByName(c.MasterName)
	if err != nil {
		return fmt.Errorf("cannot found master link by name %s", c.MasterName)
	}
	vlanName := generateVlanDeviceName(master.Attrs().Name, c.Vid)
	...
	if c.HardwareAddr.String() != "" {
		vlan.Attrs().HardwareAddr = c.HardwareAddr
	}
	...
	return netNS.Do(func(netNS ns.NetNS) error {
		...
		if contLink.Attrs().Name == c.IfName {
			return nil
		}
		return netlink.LinkSetName(contLink, c.IfName)
