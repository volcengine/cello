...
func (o *ExecMetaShimFactory) ExecCMDAdd(name string, args *skel.CmdArgs) ([]byte, error) {
	...
	lg.InfoS("ExecCMDAdd time costMillisecond", "cost", fmt.Sprintf("%f", duration), "name", name)

	if err != nil {
		lg.ErrorS(err, "ExecCMDAdd error", "name", name)
		return nil, err
	}
...
func (o *ExecMetaShimFactory) ExecCMDDel(name string, args *skel.CmdArgs) ([]byte, error) {
	...
	duration := metrics.MsSince(start)
	lg.InfoS("ExecCMDDel time cost Millisecond", "cost", fmt.Sprintf("%f", duration), "name", name)
	if err != nil {
		lg.ErrorS(err, "ExecCMDDel error", "name", name)
	}
	return nil, err
}
