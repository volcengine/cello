...
import (
	...
	celloGrpc "github.com/volcengine/cello/pkg/plugins/client/grpc"
	"github.com/volcengine/cello/pkg/plugins/client/kubelet"
	"github.com/volcengine/cello/pkg/plugins/log"
	types2 "github.com/volcengine/cello/pkg/plugins/types"
...
const (
	...
	defaultCNIMetaPodInfoDir = "/var/run/cello-meta/pod-info"
...
var lg = log.Log.WithFields(logger.Fields{"component": "cello-meta CNI"})
...
var (
	...
	// resourceClient kubelet resource client, cache response inside, not threadsafe
	resourceClient types2.ResourceClient
	patchStatus    bool
)
...
type MetaConfig struct {
	// IPRanges is ip ranges for ipam
	IPRanges []RangeSet `json:"ipRanges,omitempty"`
	// DeviceID is device id for network interface, mostly is pciAddress
	DeviceID string `json:"deviceID,omitempty"`
	// OriginalDeviceID is original device id for network interface
	OriginalDeviceID string `json:"originalDeviceId,omitempty"`
}
...
func loadCelloClient(ctx context.Context) error {
	...
	celloClient, conn, err = celloGrpc.NewCelloClient(ctx)
	if err != nil {
		return err
	}
	return nil
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	lg.InfoS("cmdAdd", "cniConfig", cniConfig, "k8sConfig", k8sConfig)

	// set log level
	if len(cniConfig.LogLevel) != 0 {
		lg.SetLogLevel(cniConfig.LogLevel)
	}
	...
	for idx, d := range delegates {
		...
		if resErr != nil {
			lg.ErrorS(resErr, "DelegateAdd: error converting result")
		...
		if d.PodNetwork.Traffic != nil {
			if len(d.PodNetwork.Traffic.Routes.ExtraRoutes) != 0 {
				lg.InfoS("add traffic route", "name", d.Name)
	...
	if patchStatus {
		lg.InfoS("patch status", "netStatus", netStatus)
...
func setupRoutes(nsname string, netInfos []NetNsConfig) error {
	...
	lg.InfoS("setup Routes", "NetNsConfig", netInfos)

	defer func(netNs ns.NetNS) {
		inErr := netNs.Close()
		if inErr != nil {
			lg.Errorf("Failed to close netns due to: %v", inErr)
		}
	}(netNs)
	...
	for _, netInfo := range netInfos {
		...
		err = netNs.Do(func(netNS ns.NetNS) error {
			...
			if nErr != nil {
				lg.ErrorS(nErr, "link port failed")
				return err
			}
			tmpLink = link
			...
			for _, netIp := range netInfo.Ips {
				...
				lg.InfoS("add policy-route", "ip", netIp.Address.String())
				...
				if err != nil {
					lg.ErrorS(err, "link add ipv4 rule failed")
					return err
				}
			}
		...
		for _, route := range netInfo.ExtraRoutes {
			...
			if parseErr != nil {
				lg.ErrorS(parseErr, "parse dst error", "Dst", route.Dst)
			...
			if len(route.Gw) != 0 {
				...
				if gw == nil {
					lg.ErrorS(nil, "gw ParseIP error", "Gw", route.Gw)
	...
	err = netNs.Do(func(netNS ns.NetNS) error {
		for cidr, routes := range customRoutes {
			...
			if len(routes) == 1 {
				lg.InfoS("add ExtraRoute", "cidr", cidr, "gw", r.Gw.String())
				rt := &netlink.Route{
				...
				if replaceErr := netlink.RouteReplace(rt); replaceErr != nil {
					if !os.IsExist(replaceErr) {
						lg.ErrorS(replaceErr, "add route(dst) error", "cidr", cidr)
						return replaceErr
					}
				}
				if r.TableId > 0 {
					...
					if err != nil {
						lg.ErrorS(err, "add route  failed")
						return err
					}
				}
			}
...
func tryLoadDelegateNetConfFromAnno(ctx context.Context, cniConfig *MetaNetConf, k8sConfig *types2.K8SArgs) ([]*DelegateNetConf, error) {
	...
	if len(cniConfig.RuntimeConfig.PodAnnotations) != 0 {
		...
		lg.InfoS("Call celloClient GetPodMetaInfo rpc method")
	...
	if len(podAnno) != 0 && len(podAnno[podNetworkDefinitionKey]) != 0 {
		lg.InfoS("load annotation", "podNetworksDefinitions",
...
func verifyAndCompleteDelegateNetConfForCNIDel(ds []*DelegateNetConf) error {
	for _, d := range ds {
		if len(d.MetaRequest.IPRangesSource) != 0 && len(d.MetaConfig.IPRanges) == 0 {
			...
			lg.InfoS("Inject fake ipRanges for delegate ipRangesSource to delete", "fake", fake, "name", d.Name, "IPRangesSource", d.MetaRequest.IPRangesSource)
...
func patchNetworkStatus(ctx context.Context, k8sArgs *types2.K8SArgs, netStatus []*NetworkStatus) error {
	...
	lg.InfoS("Call celloClient PatchPodAnnotation rpc method", "networkStatus", networkStatus)
...
func createCNIRuntimeConf(args *skel.CmdArgs, k8sArgs *types2.K8SArgs, netConf *MetaNetConf, d *DelegateNetConf, ifName string) (*libcni.RuntimeConf, error) {
	...
	lg.InfoS("Create cni runtimeConf", "runtimeConf", rc)
	return rc, nil
}
...
func loadDelegateNetConf(podNamespace, podName string, pn *PodNetwork) (*DelegateNetConf, error) {
	...
	lg.DebugS("Load DelegateNetConf", "delegateConf", delegateConf)
...
func buildMetaConfigForDelegateNetConf(podNamespace, podName string, delegate *DelegateNetConf) error {
	if delegate.MetaRequest.ResourceName != "" {
		...
		if podName != "" && podNamespace != "" {
			...
			lg.DebugS("show Found resourceName annotation", "annotation", delegate.MetaRequest.ResourceName)
			resourceMap, err := getPodResourceMap(podNamespace, podName)
			if err != nil {
				return fmt.Errorf("get pod resource map failed, %v", err)
			}
			lg.DebugS("show resourceMap instance", "resourceMap", resourceMap)

			entry, ok := resourceMap[delegate.MetaRequest.ResourceName]
			if ok {
				lg.InfoS("Found device entry", "entry", entry)
				if idCount := len(entry.DeviceIDs); idCount > 0 && idCount > entry.Index {
					delegate.MetaConfig.DeviceID = entry.DeviceIDs[entry.Index]
					delegate.MetaConfig.OriginalDeviceID = delegate.MetaConfig.DeviceID
					lg.InfoS("show podName and deviceID", "podName", podName, "DeviceID", delegate.MetaConfig.DeviceID)
					if strings.Contains(delegate.MetaConfig.DeviceID, "@") {
						// device id map translate
						delegate.MetaConfig.DeviceID = strings.Split(delegate.MetaConfig.DeviceID, "@")[0]
						lg.InfoS("Normalize deviceID", "oldDevice", delegate.MetaConfig.OriginalDeviceID, "newDeviceID", delegate.MetaConfig.DeviceID)
					}
	...
	if delegate.MetaRequest.IPRangesSource != "" {
		...
		case IpRangeSourceNetDeviceSubnet, IPRangeSourceNetDeviceSelf:
			if delegate.MetaConfig.DeviceID == "" {
				...
				lg.ErrorS(nil, "Delegate meta ipRangesSource got device id empty, ignore")
				break
			}
			var err error
		...
		default:
			lg.ErrorS(nil, "Delegate meta ipRangesSource unknown, ignored", "IPRangesSource", delegate.MetaRequest.IPRangesSource)
		}
	}
...
// loadResourceClient resource client cache response inside
func loadResourceClient() (types2.ResourceClient, error) {
	if resourceClient != nil {
		return resourceClient, nil
	}

	ck, err := kubelet.GetResourceClient("")
	if err != nil {
...
func getPodContainerResourceMap(podNamespace, podName string) ([]*types2.ContainerResource, error) {
	...
	return ck.GetPodContainerResourceMap(podNamespace, podName)
}
...
func cmdDel(args *skel.CmdArgs) error {
	...
	lg.InfoS("cmdDel", "cniConfig", cniConfig, "k8sConfig", k8sConfig)
	defer func() {
		if conn != nil {
			_ = conn.Close()
		}
	...
	if err != nil {
		lg.ErrorS(err, "try LoadDelegateNetConf From Cache error")
	}
...
func delPlugins(args *skel.CmdArgs, k8sArgs *types2.K8SArgs, netConf *MetaNetConf, delegates []*DelegateNetConf, lastIdx int) error {
	...
	for idx := lastIdx; idx >= 0; idx-- {
		...
		lg.InfoS("DelegateDel", "name", delegates[idx].Name, "ifName", ifName, "idx", idx)
		// Attempt to delete all but do not error out, instead, collect all errors.
		if err = DelegateDel(rt, delegates[idx], netConf); err != nil {
			lg.ErrorS(err, "DelegateDel error", "name", delegates[idx].Name, "ifName", ifName, "idx", idx)
			errorstrings = append(errorstrings, err.Error())
		}
	}
...
func conflistDel(rt *libcni.RuntimeConf, delegate *DelegateNetConf, netConf *MetaNetConf) error {
	// In part, adapted from K8s pkg/kubelet/dockershim/network/cni/cni.go
	binDirs := filepath.SplitList(os.Getenv("CNI_PATH"))
	binDirs = append([]string{defaultBinDir}, binDirs...) //TODO: Deduplication
	cniNet := libcni.NewCNIConfigWithCacheDir(binDirs, defaultCNIDir, NewMetaExec())
