...
import (
	...
	"github.com/volcengine/cello/pkg/plugins/device/ipvlan"
...
func (d *IPVlanDriver) SetupNetwork(config *types.SetupConfig) (err error) {
	...
	if err != nil {
		ipvlanLg.ErrorS(err, "IPVlanConf setup error")
		return
	}
	...
	if config.SetupInitNs {
		// 3. setup init ns
		ipvlanLg.Infof("SetupInitNamespace")
		err = d.setupInitNamespace(config)
		if err != nil {
			ipvlanLg.ErrorS(err, "SetupInitNamespace failed")
			return err
		}
	}
	return
...
func (d *IPVlanDriver) setupFilters(link netlink.Link, srcEgressRedirectCIDRs []*net.IPNet, egressIndex int,
	...
	if err != nil {
		ipvlanLg.ErrorS(err, "failed to get filter list")
	}
...
func (d *IPVlanDriver) createIPVlanSlave(parentLink netlink.Link, slaveName string) (netlink.Link, error) {
	...
	if err != nil {
		...
		ipvlanLg.InfoS("Slave interface has existed.")
	...
	ipvlanLg.InfoS("Create slave device in host netns success", "slaveName", slaveName)
	return link, nil
}
