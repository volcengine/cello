...
import (
	...
	"github.com/volcengine/cello/pkg/plugins/device/veth"
	"github.com/volcengine/cello/pkg/plugins/device/vlan"
...
var vlanLg = log.Log.WithFields(logger.Fields{"component": "vlan driver"})
...
func (d *VlanDriver) SetupNetwork(cfg *types.SetupConfig) (err error) {
	...
	defer func(netNs ns.NetNS) {
		inErr := netNs.Close()
		if inErr != nil {
			ipvlanLg.ErrorS(inErr, "Failed to close netns")
		}
	}(netNs)
	...
	err = netNs.Do(func(_ ns.NetNS) error {
		...
		inErr = device.Setup(podLink, containerConf)
		if inErr != nil {
			return inErr
		}
		...
		if setVeth {
			...
			inErr = vethHost.Setup(hostNetNS)
			if inErr != nil {
				return fmt.Errorf("setup vethHost pair error, %s", inErr.Error())
			}

			veth1, dErr := netlink.LinkByName(defaultVethForContainer)
			if dErr != nil {
				return dErr
			}
			vethConf, dErr := generateVethConf(cfg, veth1)
			if dErr != nil {
				return dErr
			}
			return device.Setup(veth1, vethConf)
		}
		return nil
	})
	if err != nil {
	...
	vlanLg.InfoS("Container ns setup success")
	// 3. fast path to pod
	...
	vlanLg.InfoS("Setup filters")
	// 4. check tc in parent device
	return d.setupFilters(cfg, parentENI)
}

func generateVethConf(cfg *types.SetupConfig, link netlink.Link) (*device.Conf, error) {
...
func generateVlanConf(cfg *types.SetupConfig, link netlink.Link) (*device.Conf, error) {
